...
/* ===== Utilidades ===== */
const toLcTrim = (s) => (s ?? "").trim().toLowerCase();
...
const toRecordIfObject = (value) => {
    if (!value || typeof value !== "object" || Array.isArray(value)) {
        return null;
    }
    return value;
};
const collectKeywordCandidates = (text, interactiveTitle, interactiveId) => {
...
const findBestMatchingFlow = (flows, context) => {
    if (!flows.length) {
        return null;
    }
    const keywordCandidates = collectKeywordCandidates(context.fullText, context.interactiveTitle, context.interactiveId);
    ...
    for (const flow of flows) {
        ...
        if (normalizedTrigger && !isDefaultTrigger) {
            ...
            else if (normalizedInteractiveTitle &&
                normalizedInteractiveTitle.includes(normalizedTrigger)) {
                matchesTrigger = true;
            }
            else if (normalizedInteractiveId &&
        ...
        if (!matchesTrigger && isDefaultTrigger)
            score += 1;
        if (score <= 0) {
            continue;
        }
...
function extractStatusError(errors) {
    if (!errors?.length)
        return null;
    const [first] = errors;
    if (!first)
        return null;
    const details = typeof first.error_data === "object" && first.error_data
...
function parseStatusTimestamp(timestamp) {
    if (!timestamp)
        return null;
...
async function adjustBroadcastAggregates(broadcastId, previousStatus, nextStatus) {
...
function indexWhatsappContacts(contacts) {
    const map = new Map();
    ...
    for (const contact of contacts) {
        ...
        if (!waId)
            continue;
        const profileName = typeof contact?.profile?.name === "string"
            ? contact.profile.name.trim()
            : undefined;
        const fallbackName = typeof contact?.name === "string" ? contact.name.trim() : undefined;
...
async function resolveUserForPhoneNumber(phoneNumberId) {
...
async function recordSessionSnapshot(sessionId) {
    ...
    const logStatus = LOG_STATUS_MAP[sessionSnapshot.status] ??
...
async function getOrCreateContactForUser(userId, phone, options = {}) {
    ...
    for (const candidate of options.alternatePhones ?? []) {
        ...
        if (!trimmedCandidate)
            continue;
        const normalizedCandidate = normalizePhone(trimmedCandidate) ?? trimmedCandidate;
    ...
    const normalizedName = typeof options.name === "string" && options.name.trim().length
        ? options.name.trim()
        : null;
    if (!contact) {
        try {
...
async function ensureActiveSessionForFlow(contact, flow) {
...
async function handleIncomingWhatsappMessage(userId, message, contactProfile) {
    const fromRaw = typeof message.from === "string" ? message.from : "";
    const from = fromRaw.trim();
    console.log(`Handling message ${message.id} from ${fromRaw || "unknown"} of type ${message.type}.`);
    if (!from) {
    ...
    if (!textRaw && !fallbackText) {
        console.log(`Ignoring message ${message.id} (no text could be extracted).`);
        return;
    }
    ...
    let session = existingSession && isWhatsappChannel(existingSession.flow?.channel ?? null)
        ? existingSession
        : null;
    ...
    if (!flow) {
        ...
        const availableFlows = availableFlowsRaw.filter((candidate) => isWhatsappChannel(candidate.channel ?? null));
        ...
        if (!flow) {
            console.error(`No active flow found for user ${userId} to handle message ${message.id}.`);
            return;
        }
    }
    try {
        ...
        console.error(`Failed to create or resume session for contact ${contact.id}:`, error);
        return;
    }
    if (!session) {
        console.error(`Session could not be resolved for contact ${contact.id}, skipping flow execution.`);
        return;
    }
    ...
    try {
        ...
        console.error(`Failed to update message ${message.id} with session ID:`, error);
    ...
    try {
        await executeFlow(session, text, (uid, to, payload) => sendMessage(uid, to, payload, {
            contactId: contact.id,
        ...
        try {
            ...
            console.error(`Failed to mark session ${session.id} as errored after execution failure:`, updateError);
        }
    }
    finally {
        ...
        try {
            ...
            console.error(`Failed to record session snapshot for ${session.id}:`, snapshotError);
        }
    }
}
async function processMessageStatuses(userId, statuses) {
    for (const status of statuses) {
        if (!status)
            continue;
        const messageId = typeof status.id === "string" && status.id.trim().length > 0
            ? status.id.trim()
            : null;
        if (!messageId)
            continue;
        try {
            ...
            if (nextStatus === "Failed") {
                const errorMessage = extractStatusError(status.errors ?? null) ??
            ...
            if (updatedMessage.waMessageId) {
                emitMessageStatus(updatedMessage.contactId, updatedMessage.id, updatedMessage.waMessageId, updatedMessage.status);
            }
        }
        catch (error) {
            console.error("Failed to process message status update for message:", messageId, error);
        }
    }
}
async function processBroadcastStatuses(userId, statuses) {
    for (const status of statuses) {
        if (!status)
            continue;
        const messageId = typeof status.id === "string" && status.id.trim().length > 0
        ...
        try {
            ...
            console.error("Failed to process broadcast status update for message:", messageId, error);
        }
    }
}
/**
...
function extractUserText(msg) {
    ...
    switch (msg.type) {
        ...
        case "interactive": {
            ...
            const fallbackId = msg.interactive?.button_reply?.id?.trim() ??
...
const extractWebhookChangeValues = (payload) => {
    if (!payload || typeof payload !== "object") {
        return [];
    }
    const changeValues = [];
    const entries = "entry" in payload && Array.isArray(payload.entry) ? payload.entry : null;
    if (entries?.length) {
    ...
    const standaloneValue = "value" in payload && payload.value && typeof payload.value === "object"
        ? payload.value
        : null;
    if (standaloneValue) {
        changeValues.push(standaloneValue);
    }
    return changeValues;
};
...
export async function processManualFlowTrigger(options) {
    ...
    const incomingRawText = typeof options.incomingMeta?.rawText === "string"
        ? options.incomingMeta.rawText.trim()
        : null;
    const interactiveTitle = typeof options.incomingMeta?.interactive?.title === "string"
        ? options.incomingMeta.interactive.title.trim()
        : null;
    const interactiveId = typeof options.incomingMeta?.interactive?.id === "string"
        ? options.incomingMeta.interactive.id.trim()
        : null;
    const candidateMessage = trimmedMessage && trimmedMessage.length > 0
        ? trimmedMessage
    ...
    let contact;
    try {
        const alternates = rawPhone && rawPhone !== normalizedPhone ? [rawPhone] : [];
        ...
        console.error(`Failed to resolve contact for manual trigger on flow ${flow.id}:`, error);
        return {
            success: false,
            status: 500,
    ...
    try {
        ...
        console.error(`Failed to initialise session for contact ${contact.id} on flow ${flow.id}:`, error);
        return {
            success: false,
            status: 500,
    ...
    if (variables && Object.keys(variables).length > 0) {
        const currentContext = activeSession.context ?? {};
        const nextContext = {
            ...currentContext,
            ...variables,
        };
    ...
    try {
        await executeFlow(activeSession, candidateMessage, (uid, to, payload) => sendMessage(uid, to, payload, {
            contactId: contact.id,
        ...
        const statusCode = error instanceof FlowSendMessageError && error.status
            ? error.status
            : 500;
        const errorMessage = error instanceof FlowSendMessageError && error.message
            ? error.message
...
async function addRecipientToAllowList(accessToken, phoneNumberId, recipientPhone) {
    ...
    try {
        ...
        if (!response.ok) {
            const payload = json;
            const message = payload?.error?.error_user_msg?.trim() ||
                payload?.error?.message?.trim() ||
                raw ||
                response.statusText ||
                "Failed to add phone number to WhatsApp test allow list";
            console.error("Failed to add phone number to WhatsApp allow list:", response.status, message);
            return {
                success: false,
        ...
        console.info("Successfully added phone number to WhatsApp allow list:", recipientPhone);
        return { success: true, details: json ?? raw };
    }
    catch (error) {
        if (error instanceof Error && error.name === "AbortError") {
            console.error("Timed out while adding phone number to WhatsApp allow list", recipientPhone);
            return {
                success: false,
                error: "Request to Meta timed out while registering the recipient",
            };
        }
        console.error("Unexpected error while adding phone number to WhatsApp allow list:", recipientPhone, error);
        return {
            success: false,
...
export async function sendMessage(userId, to, message, options = {}) {
                ...
                const parameters = Array.isArray(component?.parameters)
                    ? component.parameters
                        ...
                        if (!parameter || parameter.type !== "text")
                            return null;
                        const textValue = typeof parameter.text === "string" ? parameter.text : "";
    ...
    try {
        ...
        if (!res.ok) {
            ...
            const graphMessage = errorPayload?.error?.error_user_msg ?? errorPayload?.error?.message;
            const fallback = raw || res.statusText || "Meta API request failed";
            const errorMessage = graphMessage?.trim().length
                ? graphMessage
                : fallback;
            const lowerMessage = errorMessage?.toLowerCase() ?? "";
            const isAccessTokenError = res.status === 401 ||
            ...
            const normalizedError = isAccessTokenError
                ...
                : errorMessage;
            const errorCode = typeof errorPayload?.error?.code === "number"
                ? errorPayload.error.code
                : null;
            if (res.status === 400 &&
                errorCode === RECIPIENT_NOT_ALLOWED_ERROR_CODE &&
                !allowListAttempted) {
                console.warn("Recipient phone number not in WhatsApp allow list. Attempting automatic registration.", normalizedTo);
                const allowListResult = await addRecipientToAllowList(accessToken, phoneNumberId, normalizedTo);
                ...
                const registrationError = allowListResult.error ??
        ...
        const response = json;
        const messageId = response?.messages?.find((m) => typeof m?.id === "string")?.id ?? null;
