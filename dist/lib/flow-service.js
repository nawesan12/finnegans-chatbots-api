import { z } from "zod";
import { FlowSanitizationError, emptyFlowDefinition, sanitizeFlowDefinition, } from "./flow-schema";
...
const toJsonValue = (value) => {
    try {
        ...
        throw new FlowValidationError("Meta flow metadata must be JSON serializable", 400);
    }
};
...
const toFlowResource = (flow) => {
    ...
    try {
        ...
        if (error instanceof FlowSanitizationError) {
            console.warn(`Failed to sanitize flow definition for flow ${flow.id}: ${error.message}`);
...
const prepareMetaFlowCreate = (payload) => {
    if (!payload) {
        return {};
    }
    const result = {};
    ...
    if (id !== null)
        result.metaFlowId = id;
    if (token !== null)
        result.metaFlowToken = token;
    if (version !== null)
        result.metaFlowVersion = version;
    if (revisionId !== null)
        result.metaFlowRevisionId = revisionId;
    if (status !== null)
        result.metaFlowStatus = status;
    if (hasOwn(payload, "metadata")) {
        result.metaFlowMetadata = payload.metadata == null ? null : toJsonValue(payload.metadata);
    }
    return result;
};
const prepareMetaFlowUpdate = (payload) => {
    if (payload === null) {
        return {
            metaFlowId: null,
            metaFlowToken: null,
...
export async function createFlowForUser(userId, payload) {
...
export async function updateFlowById(flowId, payload) {
