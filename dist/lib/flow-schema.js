...
const ensureCoordinate = (value) => {
    if (typeof value === "number" && Number.isFinite(value))
        return value;
    const coerced = Number(value);
    return Number.isFinite(coerced) ? coerced : 0;
};
const parseFlowInput = (input) => {
    if (typeof input === "string") {
        ...
        try {
            ...
            throw new FlowSanitizationError("Flow definition string must be valid JSON.");
        }
    }
    return input ?? {};
};
export const sanitizeFlowDefinition = (input) => {
    ...
    const nodes = parsed.nodes.map((node) => {
        ...
        return {
            ...
            position: {
                x: ensureCoordinate(position.x),
                y: ensureCoordinate(position.y),
            },
        };
    });
...
export const emptyFlowDefinition = { nodes: [], edges: [] };
