...
const ensureCredentials = async (userId) => {
    ...
    if (!accessToken || !wabaId) {
        throw new MetaFlowError("Missing Meta credentials. Configure Access Token and Business Account ID in Settings.", { status: 400 });
    }
...
const performRequest = async (credentials, method, body) => {
    ...
    try {
        ...
        if (!response.ok) {
            const graphMessage = typeof json === "object" && json !== null && "error" in json
                ? (json
                    .error?.error_user_msg ??
                    json.error?.message)
                : undefined;
            const message = (typeof graphMessage === "string" && graphMessage.trim()) ||
                response.statusText ||
...
export const createMetaFlow = async (userId, payload) => {
...
export const updateMetaFlow = async (userId, flowId, payload) => {
    if (!flowId?.trim()) {
...
export const deleteMetaFlow = async (userId, flowId) => {
    if (!flowId?.trim()) {
...
//# sourceMappingURL=meta-flow.js.map